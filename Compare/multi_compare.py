# -*- coding: utf-8 -*-
"""
Created on Mon Aug 19 08:54:05 2019
...
if __name__ == '__main__':
    MW_data_folder = r"D:\wxzx\testdata\0604MW_data"+"\\"
    MW_real_folder = r"D:\wxzx\testdata\0604MW_real"+"\\"
    crtm_folder = r"D:\wxzx\testdata\0604txt" + "\\"
    ...
    for j in range(2,247,40):    
        ...
        plt.savefig(r'D:\wxzx\testdata\0604pic'+'\\'+str(j)+'.png', dpi=300)
        plt.show()
    ...
    map1 = Basemap(llcrnrlon=sub_lola[0],llcrnrlat=sub_lola[2],urcrnrlon=sub_lola[1],urcrnrlat=sub_lola[3],resolution='i', \
             projection='tmerc',lat_0 = (sub_lola[2]+sub_lola[3])/2, lon_0 = (sub_lola[0]+sub_lola[1])/2)
    map1.drawcoastlines()
    map1.scatter(np.array(MW_datas[i].iloc[:,1]), np.array(MW_datas[i].iloc[:,0]), c=crtm_MW_errors[i][0,:,draw_channel-1],s = 15, alpha = .5, latlon=True)
    plt.colorbar()
    ...
    map1 = Basemap(llcrnrlon=sub_lola[0],llcrnrlat=sub_lola[2],urcrnrlon=sub_lola[1],urcrnrlat=sub_lola[3],resolution='i', \
             projection='tmerc',lat_0 = (sub_lola[2]+sub_lola[3])/2, lon_0 = (sub_lola[0]+sub_lola[1])/2)
    map1.drawcoastlines()
    map1.scatter(np.array(MW_datas[i].iloc[:,1]), np.array(MW_datas[i].iloc[:,0]), c=crtm_MW_errors[i][1,:,draw_channel-1],s = 15, alpha = .5, latlon=True)
    plt.colorbar()
    ...
    map1.scatter(np.array(MW_datas[i].iloc[:,1]), np.array(MW_datas[i].iloc[:,0]), c=crtm_MW_errors[i][2,:,draw_channel-1],s = 15, alpha = .5, latlon=True)
    plt.colorbar()    
    plt.title('channel%d'%draw_channel)
    plt.show()    
