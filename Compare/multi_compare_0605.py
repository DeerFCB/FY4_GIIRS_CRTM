...
def read_crtm(crtm_path, point_num, channel):
    ...
    crtm_real_bt = np.array( crtm_data[:,channel,2])
    crtm_real = crtm_real.reshape(crtm_real.shape[0],crtm_real.shape[1])
    crtm_real_bt = crtm_real_bt.reshape(crtm_real.shape[0],crtm_real.shape[1])
    
    return crtm_real, crtm_real_bt

def rad2bt(MW_real, channel):
    H = 6.2373e-27
    C = 2.99791e+10
    B = 1.38024e-16
    C1 = 2.*H*C*C
    C2 = H*C/B
    wns = 1650. + (channel-1)*0.625
    MW_real_BT = np.zeros((MW_real.shape[0],MW_real.shape[1]))
    for i in range(len(channel)):
        wn = wns[i]
        MW_real_BT[:,i] = C2*wn/(np.log(C1*wn*wn*wn/MW_real[:,i]+1))
    return MW_real_BT
...
if __name__ == '__main__':
    ...
    #for i in range(1,2):
        ...
        MW_real_bt = rad2bt(MW_real, channel)
        crtm_real, crtm_real_bt = read_crtm(crtm_paths[i], MW_data.shape[0], channel)
        crtm_MW_error = np.array([MW_real, crtm_real, crtm_real - MW_real])
        crtm_MW_bt_error = np.array([MW_real_bt, crtm_real_bt, crtm_real_bt - MW_real_bt])
        
        MW_datas.append(MW_data)
        crtm_MW_errors.append(crtm_MW_error)
        crtm_MW_bt_errors.append(crtm_MW_bt_error)
